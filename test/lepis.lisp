...
(deftest test-zinterstore ()
  (with-db ("/tmp/lepis/")
    ...
    (is (= 2 (zinterstore :z3 '((:z1 2) :z2) :aggregate #'*)))
...
(deftest test-zinter ()
  (with-db ("/tmp/lepis/")
    ...
    (zadd :z1 1 'foo 2 'bar 3 'baz)
    (zadd :z2 20 'bar 30 'baz 40 'foz)
    (is (equal '((bar . 80) (baz . 180))
               (zinter '((:z1 2) :z2) :aggregate #'* :start 0 :with-scores t)))
    (is (equal '(baz bar)
               (zinter '((:z1 2) :z2) :aggregate #'* :start 0 :from-end t)))))

(deftest test-zunionstore ()
  (with-db ("/tmp/lepis/")
    (clear-db)
    (zadd :z1 1 'foo 2 'bar 3 'baz)
    (zadd :z2 20 'bar 30 'baz 40 'foz)
    (is (= 4 (zunionstore :z3 '((:z1 2) :z2) :aggregate #'*)))
    (is (equal '((foo . 2) (foz . 40) (bar . 80) (baz . 180)) (zrang :z3 0 nil :with-scores t)))
    (zunionstore :z3 '(:z99999 :z2) :aggregate #'*)
    (is (equal '((bar . 20) (baz . 30) (foz . 40))
               (zrang :z3 0 nil :with-scores t)))))
...
(deftest test-zunion ()
  (with-db ("/tmp/lepis/")
    ...
    (zadd :z1 1 'foo 2 'bar 3 'baz)
    (zadd :z2 20 'bar 30 'baz 40 'foz)
    (is (equal '((foo . 2) (foz . 40) (bar . 80) (baz . 180))
               (zunion '((:z1 2) :z2) :aggregate #'* :start 0 :with-scores t)))
    (is (equal '( foz baz bar)
               (zunion '(:z99999 :z2) :aggregate #'* :start 0 :from-end t)))))
